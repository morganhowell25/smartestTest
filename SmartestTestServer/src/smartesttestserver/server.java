...
public class server {
    ...
    public static void updateTestLOs(String pincode, String cat1, String cat2, Boolean right) {
        ...
        String query3 = "UPDATE tbl_testLOs SET correct='" + correct + "' WHERE pincode='" + pincode + "' AND cat1='" + cat1 + "' AND cat2='" + cat2 + "';";
        execNonQuery(query3);
        String query4 = "UPDATE tbl_testLOs SET total='" + total + "' WHERE pincode='" + pincode + "' AND cat1='" + cat1 + "' AND cat2='" + cat2 + "';";
        execNonQuery(query4);
    }
    ...
    public static void viewStudentScores(String pincode) {
        ...
        String query = "SELECT tbl_user.id FROM tbl_user JOIN tbl_gradedtest ON tbl_gradedtest.sid = tbl_user.id"
                + " WHERE pincode='" + pincode + "';";
        ArrayList<String> arrSSLStruct = execQuery(query);
        String query2 = "SELECT uname FROM tbl_user JOIN tbl_gradedtest ON tbl_gradedtest.sid = tbl_user.id"
                + " WHERE pincode='" + pincode + "';";
        ArrayList<String> arrSSLStruct2 = execQuery(query2);
        String query3 = "SELECT score FROM tbl_user JOIN tbl_gradedtest ON tbl_gradedtest.sid = tbl_user.id"
                + " WHERE pincode='" + pincode + "';";
        ArrayList<String> arrSSLStruct3 = execQuery(query3);
        ...
        for (int i = 0; i < arrSSLStruct.size(); i++) {
            ...
            returnStr.add(arrSSLStruct.get(i));
            returnStr.add(arrSSLStruct2.get(i));
            returnStr.add(arrSSLStruct3.get(i));
            rs.add(returnStr);
        }
        ...
        System.out.println(utils.toStr(rs));
    }
    ...
    public static void saveTest(String pincode, int teacherID, String myTest) {
        ...
        String query = "INSERT INTO tbl_test (pincode, tid, testObj) VALUES ('" + pincode + "', '"
                + teacherID + "', '" + myTest + "');";
        /*Test t = (Test)utils.toObj(myTest);
        Question[] questions = t.getTestQuestions();
        for(int i = 0; i < questions.length; i++)
        {
            for(int j = 0; j < questions[i].getLOs().size(); j++)
            {
            query += "INSERT INTO tbl_testLOs (pincode, cat1, cat2, correct, total) VALUES ('"+pincode+"','" + questions[i].getLOs().get(0).get(j) + "', '" + questions[i].getLOs().get(1).get(j) + "', '0', '0');";
            }
        }*/
        execNonQuery(query);
    }
    ...
    public static void uploadOneTestLO(String pincode, String cat1, String cat2) {
        String query = "INSERT INTO tbl_testLOs (pincode, cat1, cat2, correct, total) VALUES ('" + pincode + "','" + cat1 + "', '" + cat2 + "', '0', '0');";
        execNonQuery(query);
    }
    ...
    public static void main(String[] args) {
        ...
        switch (op) {
            ...
            case "pullID":
                pullID(args[1]);
                break;
            case "uploadOneTestLO":
                uploadOneTestLO(args[1], args[2], args[3]);
                break;
        }
    }
}
